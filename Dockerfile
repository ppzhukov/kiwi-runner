...
RUN apt update
RUN apt -y install systemd debootstrap qemu-utils mtools xorriso gdisk rsync kpartx dosfstools jing python3-pip gnupg-agent
...
RUN bash -c 'cat <<EOF > /etc/kiwi.yml
mapper:
    - part_mapper: kpartx
EOF'
